...
token = "9ds8F7N7Qjal5PoxarEp8ctl32M3lvsN"    # 项目的 token
cluster = "volcengine_input_common"  # 请求的集群
audio_path = "../../recorded.wav"  # 本地音频路径
...
def execute_one(audio_path):
    """
    :param audio_path: 音频文件地址
    :return: 语音识别结果的 text 部分
    """
    audio_id = str(uuid.uuid4())  # 生成一个随机的 ID
    ...
    if 'payload_msg' in result and 'result' in result['payload_msg']:
        text_result = result['payload_msg']['result'][0]['text']
        return text_result
    else:
        return "识别失败"
